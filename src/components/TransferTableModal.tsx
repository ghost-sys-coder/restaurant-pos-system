...
export default function TransferTableModal({ order, source, onClose }: { order: Order; source: RestaurantTable; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm"><section role="dialog" aria-modal="true" aria-labelledby="transfer-title" className="w-full max-w-md rounded-2xl bg-white shadow-2xl"><header className="flex items-center justify-between border-b p-5"><div><h2 id="transfer-title" className="font-bold">Transfer active order</h2><p className="text-xs text-slate-500">Move {order.orderNumber} from {source.tableNumber} without recreating it.</p></div><button onClick={onClose} className="rounded-lg p-2 hover:bg-slate-100"><X className="size-4" /></button></header><div className="space-y-4 p-5"><div className="flex items-center gap-3 rounded-xl bg-slate-50 p-3 text-sm font-semibold"><span>{source.tableNumber}</span><ArrowRight className="size-4 text-indigo-600" /><Select value={targetId ? String(targetId) : null} onValueChange={value => setTargetId(value ? Number(value) : null)}><SelectTrigger className="h-10 min-w-0 flex-1 bg-white"><SelectValue placeholder="Select destination" /></SelectTrigger><SelectContent>{targets.map(table => <SelectItem key={table.id} value={String(table.id)}>{table.tableNumber} / {table.section}{table.status === 'reserved' ? ' (reserved)' : ''}</SelectItem>)}</SelectContent></Select></div>{!targets.length && <p className="rounded-xl bg-amber-50 p-3 text-xs text-amber-800">There are no available destination tables.</p>}{error && <p role="alert" className="rounded-xl bg-rose-50 p-3 text-xs text-rose-700">{error}</p>}<div className="flex gap-2"><button onClick={onClose} className="flex-1 rounded-xl bg-slate-100 py-2.5 text-xs font-bold">Cancel</button><button onClick={submit} disabled={!targetId || busy} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-indigo-600 py-2.5 text-xs font-bold text-white disabled:opacity-50">{busy && <LoaderCircle className="size-4 animate-spin" />}Transfer order</button></div></div></section></div>;
}
