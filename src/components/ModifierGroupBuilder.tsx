...
export default function ModifierGroupBuilder({ groups, onChange }: Props) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-indigo-100 bg-indigo-50/40 max-sm:[&_.grid]:grid-cols-1 max-sm:[&_p]:max-w-none">
    <header className="flex flex-col items-stretch gap-4 border-b border-indigo-100 bg-white px-4 py-4 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex w-full gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-indigo-100 text-indigo-700"><Settings2 className="size-4" /></span><div className="min-w-0 flex-1"><h4 className="text-sm font-bold text-slate-900">Meal customizations</h4><p className="mt-1 w-full text-xs leading-5 text-slate-500">Add the choices staff can offer when this meal is ordered, such as size, sauce, or extras.</p></div></div>
      <Button type="button" variant="outline" className="w-full shrink-0 bg-white sm:w-auto" onClick={() => onChange([...groups, emptyGroup()])}><Plus />Add group</Button>
    </header>
    <div className="space-y-4 p-4">
      ...
      {groups.map((group, groupIndex) => {
        ...
        return <article key={groupIndex} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xs">
          <div className="flex flex-col items-stretch gap-3 border-b border-slate-100 bg-slate-50/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="w-full"><p className="text-[10px] font-bold uppercase tracking-wider text-indigo-600">Customization {groupIndex + 1}</p><p className="w-full text-xs text-slate-500">What choice should staff ask the customer to make?</p></div><div className="flex items-center justify-end gap-1"><Button type="button" size="icon-sm" variant="ghost" aria-label="Move customization up" disabled={groupIndex === 0} onClick={() => moveGroup(groupIndex, -1)}><ArrowUp /></Button><Button type="button" size="icon-sm" variant="ghost" aria-label="Move customization down" disabled={groupIndex === groups.length - 1} onClick={() => moveGroup(groupIndex, 1)}><ArrowDown /></Button><Button type="button" size="icon-sm" variant="ghost" aria-label="Delete customization" className="text-rose-600 hover:bg-rose-50" onClick={() => onChange(groups.filter((_, index) => index !== groupIndex))}><Trash2 /></Button></div></div>
          <div className="space-y-4 p-4">
            ...
            <button type="button" role="switch" aria-checked={required} onClick={() => updateGroup(groupIndex, { minSelections: required ? 0 : 1 })} className="flex w-full flex-col items-stretch gap-3 rounded-xl border border-slate-200 px-3 py-2.5 text-left sm:flex-row sm:items-center sm:justify-between"><span className="w-full"><span className="block text-xs font-bold text-slate-800">Customer must choose</span><span className="block w-full text-[11px] text-slate-500">Turn this on when the meal cannot be added without a selection.</span></span><span className={`relative h-6 w-11 shrink-0 self-end rounded-full transition sm:self-auto ${required ? 'bg-indigo-600' : 'bg-slate-300'}`}><span className={`absolute top-1 size-4 rounded-full bg-white transition-transform ${required ? 'translate-x-6' : 'translate-x-1'}`} /></span></button>
            <div><div className="mb-2 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="w-full"><p className="text-xs font-bold text-slate-800">Available choices</p><p className="w-full text-[11px] text-slate-500">The extra fee is optional. Leave it at zero when the choice is included.</p></div><Button type="button" size="sm" variant="outline" className="w-full sm:w-auto" onClick={() => updateGroup(groupIndex, { choices: [...group.choices, { name: '', price: 0 }] })}><Plus />Add choice</Button></div>
