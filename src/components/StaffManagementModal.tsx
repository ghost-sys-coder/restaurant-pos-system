...
export default function StaffManagementModal({ onClose }: { onClose: () => void }) {
  ...
  return createPortal(<div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-slate-950/60 p-4 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-labelledby="staff-management-title" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="my-auto flex max-h-[calc(100dvh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white text-slate-900 shadow-2xl sm:max-h-[calc(100dvh-3rem)]">
      ...
      {currentUser?.role === 'restaurant_owner' && <form onSubmit={inviteAdmin} className="p-5 grid sm:grid-cols-[1fr_180px_auto] gap-3 border-b"><div className="space-y-1"><Label>Back-office email</Label><Input type="email" value={adminEmail} onChange={e => setAdminEmail(e.target.value)} placeholder="admin@restaurant.com" /></div><div className="space-y-1"><Label>Access</Label><select value={adminRole} onChange={e => setAdminRole(e.target.value as typeof adminRole)} className="h-9 w-full rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="restaurant_admin">Restaurant Admin</option><option value="general_manager">General Manager</option><option value="accountant">Accountant</option></select></div><Button type="submit" className="self-end" disabled={!adminEmail.includes('@')}>Send invite</Button></form>}
      <form onSubmit={create} className="p-5 grid sm:grid-cols-4 gap-3 bg-slate-50 border-b">
        ...
        <div className="space-y-1"><Label>Role</Label><select value={role} onChange={e => setRole(e.target.value as Role)} className="h-9 w-full rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="shift_manager">Shift Manager</option><option value="cashier">Cashier</option><option value="server">Server</option><option value="bartender">Bartender</option><option value="host">Host</option><option value="kitchen">Kitchen</option></select></div>
