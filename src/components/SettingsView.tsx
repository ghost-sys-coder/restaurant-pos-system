...
import { Building2, Coins, Globe2, LoaderCircle, MapPin, Percent, Plus, ReceiptText, Save, Settings, ShieldCheck, Timer } from 'lucide-react';
...
export default function SettingsView() {
  ...
  return <div className="flex-1 overflow-y-auto bg-slate-50/80 px-4 py-6 lg:px-8 lg:py-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-200/40"><div className="flex items-center gap-2 border-b border-slate-100 px-5 py-4"><ShieldCheck className="size-4 text-indigo-600" /><h2 className="font-bold">Audit trail</h2></div>{events.length ? <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="bg-slate-50 text-slate-500"><tr><th className="p-3">Time</th><th className="p-3">Action</th><th className="p-3">Entity</th><th className="p-3">Actor</th></tr></thead><tbody className="divide-y">{events.map(item => <tr key={item.id} className="hover:bg-slate-50/70"><td className="whitespace-nowrap p-3 text-slate-500">{formatDate(item.createdAt)}</td><td className="p-3 font-medium">{item.action}</td><td className="p-3 text-slate-600">{item.entityType || '-'} {item.entityId || ''}</td><td className="p-3 text-slate-600">{item.actorStaffId || 'System'}</td></tr>)}</tbody></table></div> : <p className="p-8 text-center text-sm text-slate-500">No audit events recorded yet.</p>}</section>
